if global.nightsave == 1{
  time_next_move += 1
    if (time_next_move >= time_to_stay)
    {
        if (monster_camera == 1)
        {
            cameras_visited[0] = true
            monster_camera = 4
        }
        else if (monster_camera == 4)
        {
            cameras_visited[3] = true
            monster_camera = 2
        }
        else if (monster_camera == 2)
        {
            cameras_visited[1] = true
            monster_camera = 1
        }
        time_next_move = 0
	...
	if (cameras_visited[0] && cameras_visited[1] && cameras_visited[3])
{
    ...
    {
        if (!instance_exists(obj_toiletunnyoffice))
        {
            instance_create_layer(x, y, "Instances", obj_toiletunnyoffice)
            special_item_count++
        }
}
    cameras_visited = [false, false, false]
}
...
if (cameras_visited5[0] && cameras_visited5[1]) {
    ...
    if (!instance_exists(obj_blunger)) { // Verifica se o objeto ainda não foi criado
        instance_create_layer(1024, 1376, "Instances", obj_blunger); // Substitua por "RoomObjects" com o layer correto
